    ...
    <div class="col-2"><?php if (!empty($adviser['date_approved'])) {
        ...
        if ($application->userInRole(['adviser'])) { ?>
                <a class="btn btn-link"
                    onclick="clearStudent(<?= $studentId ?>,<?= $application->getAccountId() ?>,4, null)">Clear
                    Student</a>
            <?php } else {
            echo 'Pending';
        }
...
<div class="row">
    ...
    <div class="col-2"><?php if (!empty($student_affair['date_approved'])) {
        ...
        if ($application->userInRole(['student_affairs'])) { ?>
                <a class="btn btn-link"
                    onclick="clearStudent(<?= $studentId ?>,<?= $application->getAccountId() ?>,6, null)">Clear
                    Student</a>
            <?php } else {
            echo 'Pending';
        }
...
<div class="row">
    ...
    <div class="col-2"><?php if (!empty($dean['date_approved'])) {
        ...
        if ($application->userInRole(['dean'])) { ?>
                <a class="btn btn-link"
                    onclick="clearStudent(<?= $studentId ?>,<?= $application->getAccountId() ?>,5, null)">Clear
                    Student</a>
            <?php } else {
            echo 'Pending';
        }
